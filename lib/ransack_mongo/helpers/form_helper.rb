  ...
  module Helpers
    module FormHelper
      ...
      def search_form_for(record, options = {}, &proc)
        if record.is_a? RansackMongo::Search
          search = record
          options[:url] ||= polymorphic_path(
            search.klass, format: options.delete(:format)
            )
        elsif record.is_a?(Array) &&
        (search = record.detect { |o| o.is_a?(RansackMongo::Search) })
          options[:url] ||= polymorphic_path(
            options_for(record), format: options.delete(:format)
            )
        else
          raise ArgumentError,
          'No RansackMongo::Search object was provided to search_form_for!'
        end
        options[:html] ||= {}
        html_options = {
        ...
        options[:as] ||= RansackMongo.options[:search_key]
      ...
      def sort_link(search_object, attribute, *args, &block)
        ...
        unless Search === search
          raise TypeError, 'First argument must be a RansackMongo::Search!'
        end
      ...
      private
        ...
        def parse_record(object)
          return object.klass if object.is_a?(RansackMongo::Search)
          object
        end
      ...
      class SortLink
        def initialize(search, attribute, args, params)
          ...
          @hide_indicator = @options.delete(:hide_indicator) ||
                            RansackMongo.options[:hide_sort_order_indicators]
